ALTER VIEW [dbo].[BG_Inventory_Projections_CH_SlsOrderProjectionsExcluded]
AS
--Begin Query
...
SELECT TOP (100) PERCENT Item, ItemDesc1, ItemDesc2, [PO/SLS], [SHP OR RECV DT], [QTY (QOH/QTY SLS/QTY REC)], [PROJ QOH], ORD#, [VEND/CUS], [ORD DT], 
               ...
               THEN 0 WHEN ISNUMERIC(ESS) = 0 THEN 0 ELSE ESS END AS ESS
FROM  (SELECT DISTINCT 
               ...
               FROM   dbo.Z_IMINVLOC AS IM 
						...
						INNER JOIN (SELECT PL.item_no
                                   ...
                                   WHERE (PH.ord_dt > DATEADD(DAY, - 365, GETDATE())) AND (PL.ord_status <> 'X' AND (LTRIM(PL.vend_no) IN
                                                      (SELECT vend_no
                                                       FROM   dbo.BG_CH_Vendors) OR
                                                  LTRIM(PH.vend_no) = '7224'))) AS PURCH_LAST_YR ON PURCH_LAST_YR.item_no = IM2.item_no
               ...
               FROM  dbo.apvenfil_sql AS AP 
                            ...
                            INNER JOIN (SELECT DISTINCT PL.item_no
												 FROM   dbo.poordlin_sql AS PL INNER JOIN
													  dbo.poordhdr_sql AS PH ON PH.ord_no = PL.ord_no
												 WHERE (PH.ord_dt > DATEADD(DAY, - 365, GETDATE())) AND (LTRIM(PL.vend_no) IN
                                                      (SELECT vend_no
		  ...
		  AND PH.ord_status != 'X' AND PL.ord_status != 'X' AND (IM2.prod_cat NOT IN ('036', '336', '102', '037')) AND stk_loc NOT IN ('BR', 'IN', 'CAN')
               UNION ALL
               ...
               FROM  dbo.oeordhdr_sql AS OH 
                            ...
                            INNER JOIN (SELECT DISTINCT PL.item_no
                              ...
                              JOIN Z_IMINVLOC_USAGE USG ON USG.item_no = BM.item_no
               WHERE (OH.ord_type = 'O') AND (IM2.prod_cat NOT IN ('036', '336', '102', '037'))   AND (LTRIM(OH.cus_no) NOT IN (SELECT cus_no FROM BG_Projection_Customers))
