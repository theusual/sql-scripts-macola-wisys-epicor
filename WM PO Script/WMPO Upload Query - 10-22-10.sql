/*Open Order Shipping Acknowledgement*/ SELECT '838115' AS 'Supplier Acct Number', CAST(OH.oe_po_no AS int) AS 'Wal-Mart PO Number', 
                      ...
                      rtrim(cast(ltrim(OH.ord_no) AS CHAR)) AS 'Bill of Lading Number', 'Pallet ID Number' = CASE WHEN PL.palletid IS NULL 
                      THEN (CASE WHEN SH.A4GLIDENTITY < 99999 THEN ('838115F' + '0000' + CONVERT(varchar, SH.A4GLIDENTITY, 101)) 
                      ELSE ('838115F' + '000' + CONVERT(VARCHAR, SH.A4GLIDENTITY, 101)) END) ELSE PL.palletid END, 'Container Type' = CASE substring(OL.item_no, 1, 
                      2) WHEN 'SW' THEN 'BOX' ELSE 'PALLET' END, 'Container Number' = '', 'Quantity in Container' = CASE WHEN PL.quantity IS NULL 
                      THEN rtrim(cast(cast(OL.qty_ordered AS int) AS char)) WHEN PL.quantity > OL.qty_ordered THEN rtrim(cast(cast(OL.qty_ordered AS int) AS char)) 
                      ELSE rtrim(cast(cast(PL.quantity AS int) AS char)) END, 'Wal-Mart Item Part Number' = CASE WHEN OL.cus_item_no IS NULL AND rtrim(CI.cus_item_no) 
                      IS NULL THEN rtrim(OL.item_no) WHEN OL.cus_item_no IS NULL THEN rtrim(CI.cus_item_no) ELSE rtrim(OL.cus_item_no) END, rtrim(OL.item_no) 
...
WHERE     ltrim(OH.cus_no) IN ('1575', '20938') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND NOT OH.user_def_fld_4 LIKE '%ON%' AND 
                      ...
                      'WM-MOVIEDUMPBIN', 'DVD SHELF SET') AND isnumeric(OH.oe_po_no) = 1 AND 
                      NOT OH.oe_po_no LIKE '%7141840%' /*	Conditions just for Open Order Shipped	- Pull orders shipped in last 7 days from Malvern*/ AND 
                      (CONVERT(varchar, CAST(rtrim(SH.ship_dt) AS datetime), 101) > DATEADD(day, - 15, GETDATE()))
UNION ALL
/*Closed Order Shipping Acknowledgement*/ SELECT '838115' AS 'Supplier Acct Number', CAST(OH.oe_po_no AS bigint) AS 'Wal-Mart PO Number', 
                      ...
                      ELSE ('838115F' + '000' + CONVERT(VARCHAR, SH.A4GLIDENTITY, 101)) END) ELSE PL.palletid END, 'Container Type' = CASE substring(OL.item_no, 1, 
                      2) WHEN 'SW' THEN 'BOX' ELSE 'PALLET' END, 'Container Number' = '', 'Quantity in Container' = CASE WHEN PL.quantity IS NULL 
                      THEN rtrim(cast(cast(OL.qty_ordered AS bigint) AS char)) WHEN PL.quantity > OL.qty_ordered THEN rtrim(cast(cast(OL.qty_ordered AS bigint) AS char)) 
                      ELSE rtrim(cast(cast(PL.quantity AS bigint) AS char)) END, 'Wal-Mart Item Part Number' = CASE WHEN OL.cus_item_no IS NULL AND 
                      rtrim(CI.cus_item_no) IS NULL THEN rtrim(OL.item_no) WHEN OL.cus_item_no IS NULL THEN rtrim(CI.cus_item_no) ELSE rtrim(OL.cus_item_no) END, 
                      rtrim(OL.item_no) AS 'Supplier Item Part Number', rtrim(replace(replace(replace(IM.item_desc_1, ',', ' '), '"', 'in'), '''', 'ft')) 
                      AS 'Supplier Item Part Number Description', '' AS 'Wal-Mart Component Part Number', '' AS 'Supplier Component Part Number', 
                      '' AS 'Supplier Component Description', cast(OL.qty_ordered AS bigint) AS 'Supplier Quantity Ordered', 
                      'Supplier Quantity Shipped' = CASE WHEN SH.hand_chg <> OL.qty_ordered THEN cast(OL.qty_ordered AS bigint) ELSE cast(SH.hand_chg AS bigint) 
                      END, 'C' AS 'Status', 'Shipped' AS 'Ship Condition', 'I' AS 'Part Type'
...
WHERE     ltrim(OH.cus_no) IN ('1575', '20938') AND OH.ord_type = 'O' AND NOT OH.user_def_fld_4 LIKE '%ON%' AND NOT OH.user_def_fld_4 LIKE '%FX%' AND 
                      ...
                      'DVD SHELF SET') AND isnumeric(OH.oe_po_no) = 1 AND 
                      NOT OH.oe_po_no LIKE '%7141840%' /*	Conditions just for History	- Pull orders shipped in last 7 days from Malvern*/ AND (CONVERT(varchar, 
                      CAST(rtrim(SH.ship_dt) AS datetime), 101) > DATEADD(day, - 15, GETDATE()))
UNION ALL
...
WHERE     ltrim(OH.cus_no) IN ('1575', '20938') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND NOT OH.user_def_fld_4 LIKE '%ON%' AND 
                      ...
                      NOT shipping_dt IS NULL AND NOT OL.item_no IN ('ADD ON', 'BACKORDER', 'CAP EX', 'FIXTURE REQUEST', 'INITIAL DIV 01', 'INITIAL RM', 
                      'INITIAL SC', 'INITIAL WNM', 'PROTOTYPE METAL', 'PROTOTYPE PLASTIC', 'PROTOTYPE WOOD', 'REVIEW ITEM', 'SAMPLE', 'WM-PETITEMOMZONE', 
                      'WM-MOVIEDUMPBIN', 'DVD SHELF SET') AND isnumeric(OH.oe_po_no) = 1 /*	Conditions just for ack script			*/ AND SH.ord_no IS NULL AND 
                      NOT OH.oe_po_no LIKE '%7141840%'