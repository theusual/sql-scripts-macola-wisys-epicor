SELECT TOP (100) PERCENT Item, [ItemDesc1], [ItemDesc2], [PO/SLS], [PO/SLS], [SHP OR RECV DT], [QTY (QOH/QTY SLS/QTY REC)], [PROJ QOH], ORD#, [VEND/CUS], [ORD DT], CONTAINER, [CONT. SHP TO], 
...
FROM  (SELECT DISTINCT 
               ...
               SELECT BM.comp_item_no AS ITEM, BMIM.item_desc_1 AS [ItemDesc1], BMIM.item_desc_2 AS [ItemDesc2],  'SALE' AS [PO/SLS], 
                              ...
                              END AS [SHP/RECV DT],
                       CAST(OL.qty_to_ship AS INT) * BM.qty_per_par * - 1 AS QTY, '' AS [PROJ QOH], CAST(RTRIM(LTRIM(OH.ord_no)) AS VARCHAR) AS [ORDER], OH.ship_to_name AS [VEND/CUS], 
               ...
               WHERE (OH.ord_type = 'O') AND (LTRIM(OH.cus_no) NOT IN ('23033', '24033', '32300', '32401')) AND (IM2.prod_cat NOT IN ('036', '336', '102', '037')) AND 
                              ((CAST(LTRIM(OL.ord_no) AS VARCHAR) + CAST(CAST(OL.qty_to_ship AS INT) AS VARCHAR)) NOT IN
                              ...
                              ((CAST(LTRIM(OL.ord_no) AS VARCHAR) + CAST(CAST(OL.qty_to_ship AS INT) AS VARCHAR)) NOT IN
                                  ...
                                  (SELECT PL.item_no
                                   ...
                                   WHERE (PH.ord_dt > '08/01/2010') AND (PL.ord_status <> 'X') AND (LTRIM(PL.vend_no) IN
                                                  ...
                                                  LTRIM(PH.vend_no) = '7224')))) AS HolyGrail
