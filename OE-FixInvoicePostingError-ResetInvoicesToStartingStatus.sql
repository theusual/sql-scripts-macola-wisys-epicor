...
WHERE OH.inv_batch_id is null and inv_no != ''
...
SELECT * FROM oepdshdr_sql WHERE ord_no IN (SELECT ord_no FROM oeordhdr_SQL OH WHERE invoice_no != '' AND OH.inv_batch_id is null  )

BEGIN TRAN
DELETE FROM oepdshdr_sql WHERE ord_no IN (SELECT ord_no FROM oeordhdr_SQL OH WHERE invoice_no != '' AND OH.inv_batch_id is null  )
COMMIT TRAN
...
--WHERE inv_batch_id IN ('VL 2/12C')
WHERE ord_no = '  902716'

--FOR REGULAR INVOICES:
UPDATE dbo.oeordhdr_sql
SET inv_batch_id = '', selection_cd = 'S', inv_dt = NULL, inv_no = '', status = 8
WHERE inv_batch_id is null and inv_no != ''
...
SELECT inv_no, inv_batch_id, status, selection_cd, * FROM oeordhdr_sql WHERE inv_batch_id is not null
