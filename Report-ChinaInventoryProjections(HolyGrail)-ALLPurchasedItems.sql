...
CONTAINER, [CONT. SHP TO], [XFER TO], [PROD CAT], [CUS#/VEND#], CH, STORE, [PARENT ITEM (ONLY ON SALES)], OrdType2, 
...
WHERE (IM2.prod_cat NOT IN ('036', '336', '102', '037')) 
	  ...
	  AND (PURCH_LAST_YR.item_no IS NOT NULL OR (IM2.prod_cat LIKE '3%'))

UNION ALL

SELECT PL.item_no AS ITEM, IM2.item_desc_1 AS ItemDesc1, IM2.item_desc_2 AS ItemDesc2, 'PO' AS [PO/SLS], CASE WHEN NOT (PL.user_def_fld_2 IS NULL)  THEN PL.user_def_fld_2 WHEN PL.user_def_fld_2 IS NULL AND NOT (PL.extra_8 IS NULL) AND LEN(PL.extra_8) = 10 THEN CONVERT(varchar(10), 
  DATEADD(day, 28, PL.extra_8), 101) WHEN PL.user_Def_fld_2 IS NULL AND (PL.extra_8 IS NULL OR LEN(PL.extra_8) != 10) AND LTRIM(PL.vend_no) IN  (SELECT vend_no
     FROM   BG_CH_Vendors) THEN CONVERT(varchar(10), DATEADD(DAY, 90, PH.ord_dt), 101) ELSE CONVERT(varchar(10), DATEADD(DAY, 7, PH.ord_dt), 101) END AS [SHP/RECV DT], CAST(PL.qty_ordered AS int) AS QTY, '' AS [PROJ QOH], CAST(LTRIM(SUBSTRING(PL.ord_no, 1, 6)) AS VARCHAR) 
              AS [ORDER], AP.vend_name AS [VEND/CUS], CONVERT(varchar(10), PH.ord_dt, 101) AS [ORDER DATE], PL.user_def_fld_1 AS [CONTAINER INFO], PL.user_def_fld_3 AS [Container Ship To], PS.ship_to_desc AS [TRANSFER TO], IM2.prod_cat AS [PROD CAT], LTRIM(PH.vend_no) AS [CUS NO], IM2.item_note_1 AS CH, '' AS STORE, '' AS [PARENT ITEM], IM2.item_note_4 AS ESS, USG.usage_ytd, '', IM2.extra_1 AS 'ParentFlag'
...
WHERE (OH.ord_type = 'O') AND (IM2.prod_cat NOT IN ('036', '336', '102', '037')) AND (OL.loc NOT IN ('BR', 'IN', 'CAN')) 
       ...
       --Expand the list to include china prod cat items even if not purchased last year
		AND (PURCH_LAST_YR.item_no IS NOT NULL OR (IM2.prod_cat LIKE '3%'))
       --Test
       --AND BMIM.item_no = 'DRY-EP 01 BVB'
...
SELECT OL.item_no AS ITEM, IM2.item_desc_1 AS ItemDesc1, IM2.item_desc_2 AS ItemDesc2, 'SALE' AS [PO/SLS], CASE WHEN CONVERT(varchar(10), 
              ...
              USG.usage_ytd, OH.user_def_fld_3, IM2.extra_1 AS 'ParentFlag'
FROM  dbo.oeordhdr_sql AS OH INNER JOIN
              dbo.oeordlin_sql AS OL ON OL.ord_no = OH.ord_no INNER JOIN
