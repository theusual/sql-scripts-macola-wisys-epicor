ALTER VIEW BG_Shipping_OnTimeVsLate_Weekly_AllCustomers AS 
...
WHERE (CONVERT(varchar, CAST(rtrim(oh.inv_dt) AS datetime), 101) > DATEADD(day, -8, GETDATE())) AND qty_to_ship > 0 AND OH.shipping_dt IS NOT null
...
/*ELSE ltrim(pp.Pallet_UCC128) END AS [Pallet/Carton ID]*/ FROM oeordhdr_sql OH INNER JOIN
               ...
               dbo.arcusfil_sql CUS WITH (NOLOCK) ON CUS.cus_no = OH.cus_no
WHERE (shipped = 'Y' OR
               shipped IS NULL) AND (CONVERT(VARCHAR,pp.ship_dt,101) > (DATEADD(day, -8, CONVERT(VARCHAR,GETDATE(),101))) OR (pp.ship_dt IS NULL AND CONVERT(VARCHAR,OL.shipped_dt,101) > (DATEADD(day, -8, CONVERT(VARCHAR,GETDATE(),101))))) AND OL.item_no NOT LIKE '%TEST ITEM%' AND qty_to_ship > 0 AND OH.shipping_dt IS NOT NULL 
				AND OL.loc NOT IN ('BR','IN','CH','IT','CAN','GD')
