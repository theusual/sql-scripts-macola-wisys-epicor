ALTER VIEW BG_IMINVLOC_ACCUMULATORS_ONLY_ALL_LOC AS
--Created By: Bryan Gregory
--Last Updated By:  ---
--Last Updated: 10/9/2013
--Purpose:  Accumulator fields only from iminvloc_sql.  SHOWS ALL LOCATIONS.

SELECT        IM.item_no, IL.prod_cat, IL.qty_on_hand, IL.avg_cost, IL.last_cost, IL.std_cost, IL.prior_year_usage, QA.qty_allocated AS [QALL_All Loc], 
                         QU.usage_ytd AS [USG YTD_All Loc], IM.extra_1 AS [Par/Sub/Comp Flag], IM.item_desc_1, IM.item_desc_2, IL.qty_on_ord, IL.frz_qty
FROM            dbo.z_iminvloc AS IL INNER JOIN
