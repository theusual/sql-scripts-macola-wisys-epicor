--ALTER VIEW BG_SHIPPED_BUT_NOT_INVOICED AS
SELECT DISTINCT 
               TOP (100) PERCENT   MAX(CONVERT(varchar, CAST(RTRIM(OEORDHDR_SQL.shipping_dt) AS datetime), 101)) AS [Order Shipping Date],
			   MAX(CONVERT(varchar, CAST(RTRIM(SH.ship_dt) AS datetime), 101)) AS [Ship Date], OEORDHDR_SQL.ord_no AS [Order], AR.cus_name AS [Cus Name], 
               ...
               sh.item_no AS [Shipped Items], SUM(qty) AS [Qty Shipped], (unit_price * SUM(qty)) AS [line total $], 
			   MAX(OEORDLIN_SQL.user_def_fld_1) AS [Late Order Comments 1], MAX(OEORDLIN_SQL.user_def_fld_2) AS [Late Order Comments 2], OEORDHDR_SQL.cmt_2 AS [Invoicing Comment]
FROM  dbo.oeordhdr_sql AS OEORDHDR_SQL 
			   JOIN dbo.oeordlin_sql AS OEORDLIN_SQL ON OEORDHDR_SQL.ord_type = OEORDLIN_SQL.ord_type AND OEORDHDR_SQL.ord_no = OEORDLIN_SQL.ord_no 
			   JOIN  dbo.wsPikPak AS SH ON SH.Ord_no = OEORDHDR_SQL.ord_no AND SH.line_no = OEORDLIN_SQL.line_no 
			   JOIN  dbo.arcusfil_sql AS AR ON AR.cus_no = OEORDHDR_SQL.cus_no 
			   LEFT OUTER JOIN
...
WHERE (OEORDHDR_SQL.ord_type = 'O') AND (NOT (OEORDLIN_SQL.prod_cat IN ('037', '2', '036', '102', '111', '336')))  AND
               (OEORDLIN_SQL.item_no NOT LIKE '%TEST%') AND 
			   (NOT OEORDHDR_SQL.status = 9 AND (ISNUMERIC(OEORDHDR_SQL.status) = 1)) AND 
               (OEORDLIN_SQL.ord_no + OEORDLIN_SQL.item_no IN
                   (SELECT Ord_no + Item_no AS Expr1
                    FROM   dbo.wsPikPak AS pp
                    WHERE (Shipped = 'Y')) )
			   AND (ship_dt < DATEADD(DAY, - 4, GETDATE()) OR (ship_dt is null AND shipping_dt < DATEADD(day, - 80, GETDATE())))
               --AND SHSUM.qtysum = qty_to_ship 
...
SELECT TOP (100) PERCENT MAX(CONVERT(varchar, CAST(RTRIM(OEORDHDR_SQL.shipping_dt) AS datetime), 101)) AS [Order Shipping Date], 
				--'N/A' AS [Ship Date], OEORDHDR_SQL.ord_no AS [Order], AR.cus_name AS [Cus Name], 
