SELECT TOP (100) PERCENT '___' AS LN, Z_IMINVLOC.prod_cat AS Cat, CASE WHEN (IMITMIDX_SQL.extra_1) IS NULL 
               ...
               '01/01/2011', 103), GETDATE()) / 30.5)) AS CMNTH, CAST(Z_IMINVLOC.qty_on_hand - dbo.Z_IMINVLOC_QALL.qty_allocated AS Int) AS [QOH-QOA], 
               CASE WHEN z_iminvloc_usage.usage_ytd > 0 THEN CAST(((qty_on_hand + qty_on_ord) / CEILING((z_iminvloc_usage.usage_ytd / (DATEDIFF(day, 
               CONVERT(datetime, '01/01/2011', 103), GETDATE()) / 30.5)))) AS money) ELSE 0 END AS MOI, CASE WHEN (NOT (imitmidx_sql.extra_1 = 'P') OR
               ...
               '01/01/2011', 103), GETDATE()) / 30.5))), 0)) < 0) THEN 'NC' ELSE '' END AS [CHECK 90], CASE WHEN (Z_IMINVLOC.qty_on_hand > 0) 
