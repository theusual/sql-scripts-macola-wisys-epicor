...
--CREATE VIEW BG_EOD_180 AS 

--Created:	2/27/14    By:	BG
--Last Updated:	  	   By:	BG
...
WHERE (OEORDHDR_SQL.ord_type = 'O') AND (OEORDLIN_SQL.user_def_fld_3 IS NOT NULL) 
			AND OEORDLIN_SQL.user_def_fld_3 > (DATEADD(day, - 180, GETDATE()))
...
FROM  dbo.oehdrhst_sql AS OH INNER JOIN
               oelinhst_Sql AS OL ON OH.ord_type = OL.ord_type AND OH.ord_no = OL.ord_no LEFT OUTER JOIN
...
WHERE (OH.ord_type = 'O')  AND (OL.user_def_fld_3 IS NOT NULL) AND OL.user_def_fld_3 > (DATEADD(day, - 180, GETDATE())) 
