...
WHERE ((dbo.iminvtrx_sql.doc_ord_no + dbo.iminvtrx_sql.item_no + CAST(dbo.iminvtrx_sql.quantity AS varchar(15)) + LTRIM(rtrim(comment))) NOT IN
                   (SELECT doc_ord_no + item_no + CAST(quantity AS varchar(15)) + loc AS Expr1 
                    FROM   dbo.iminvtrx_sql AS iminvtrx_sql_1
                    WHERE (lev_no = '1') AND (loc <> 'IT') AND (trx_dt > DATEADD(day, - 15, GETDATE())) AND (NOT (doc_ord_no IS NULL)))) AND (dbo.iminvtrx_sql.lev_no = '1') AND 
               (dbo.iminvtrx_sql.loc = 'IT') AND (dbo.iminvtrx_sql.trx_dt > DATEADD(day, - 15, GETDATE())) AND (NOT (dbo.iminvtrx_sql.doc_ord_no IS NULL)) AND 
               (dbo.iminvtrx_sql.item_no IN
                   (SELECT item_no
