ALTER VIEW BG_Shipping_OnTimeVsLate_Yesterday_AllCustomers AS 

--Created:	06/1/12	 By:	BG
--Last Updated:	7/13/12	 By:	BG
...
SELECT MAX(CONVERT(VARCHAR(10), OH.shipping_dt, 101)) AS [ExpectedShipDt], 
...
WHERE (CONVERT(varchar, CAST(rtrim(oh.inv_dt) AS datetime), 101) > DATEADD(day, -1, GETDATE())) AND qty_to_ship > 0
       --Test order
...
SELECT MAX(CONVERT(VARCHAR(10), oh.shipping_dt, 101)) AS [ExpectedShipDt], 
               ...
               THEN OH.ship_via_cd ELSE bl.ship_via_cd END) END AS [Carrier], pp.TrackingNo[TrackingNo], 
               OH.ship_to_name AS [ShipTo], pp.item_no AS [Item], OL.loc AS [Dock], SUM(pp.qty) AS Qty, '' AS Comments
/*ELSE ltrim(pp.Pallet_UCC128) END AS [Pallet/Carton ID]*/ FROM oeordhdr_sql OH INNER JOIN
               ...
               [001].dbo.wsShipment ws WITH (NOLOCK) ON ws.shipment = pp.shipment LEFT OUTER JOIN
               [001].dbo.oebolfil_sql BL WITH (NOLOCK) ON BL.bol_no = ws.bol_no
WHERE (shipped = 'Y' OR shipped IS NULL) AND (PP.ship_dt > (DATEADD(day, -1, GETDATE())) OR OL.shipped_dt > (DATEADD(day, -1, GETDATE()))) 
               AND OL.item_no NOT LIKE '%TEST ITEM%' AND qty_to_ship > 0
GROUP BY pp.ship_dt, OH.ord_no, oh.cus_no, pp.Shipment, OH.cus_alt_adr_cd, OH.ship_to_name, BL.ship_via_cd, pp.TrackingNo, pp.Item_no, OL.loc, PP.ParcelType, OH.ship_via_cd
