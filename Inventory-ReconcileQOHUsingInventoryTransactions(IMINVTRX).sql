...
WHERE (doc_dt = '01/05/2014') AND (doc_type IN ('I'))
...
FROM dbo.iminvloc_sql IM JOIN 
		...
		FROM  dbo.iminvtrx_sql
		WHERE (doc_dt = '01/04/2014') AND (doc_type IN ( 'R'))
...
FROM dbo.iminvloc_sql IM JOIN 
		(SELECT loc, item_no, SUM(CAST(quantity AS int)) AS [qty]
		FROM  dbo.iminvtrx_sql
		WHERE (doc_dt = '01/04/2014') AND (doc_type IN ( 'I'))
		GROUP BY loc, item_no) AS TRX ON TRX.item_no = IM.item_no AND TRX.loc = IM.loc
...
SELECT loc, trx_dt, doc_dt, doc_Type, * FROM iminvtrx_sql WHERE item_no = 'BAK-619 DOORSBL               ' AND doc_dt = '01/04/2014'
