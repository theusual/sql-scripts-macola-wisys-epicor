...
WHERE (doc_dt = '10/01/2013') AND (doc_type IN ('I'))
...
FROM dbo.iminvloc_sql IM JOIN 
		...
		FROM  dbo.iminvtrx_sql
		WHERE (doc_dt = '10/01/2013') AND (doc_type IN ( 'R'))
...
SELECT loc, trx_dt, doc_dt, doc_Type, * FROM iminvtrx_sql WHERE item_no = 'BAK-619 DOORSBL               ' AND doc_dt = '10/01/2013'
