...
WHERE ltrim(OH.cus_no) IN ('1575', '20938', '122523', '25000', '35000') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND NOT OL.item_no IN ('ADD ON', 'BACKORDER', 
               ...
               NOT ((OH.ord_no + OL.item_no) IN
                   (SELECT OH.ord_no + item_no
                    ...
                    WHERE qty_to_ship > 0)) /*Exclude split shipment orders   */ AND (OH.ord_no + IM.item_no) NOT IN ('  680261BAK-695 OBV-097', '  680102BAK-ARTBRDE 97', 
               '  680261OBP-1822BSOBV97', '  680524MDWM-0015 SB', '  680524MDWM-0002 SB', '  680524MDWM-0003 SB', '  680524MDWM-0001 SB',
               '  681451BAK-707 C OBV97', '  681451BAK-707EC OBV97', '  680451BAK-ARTBRDE 97', '  681591BAK-707EC OBV97')
/*#endregion*/ UNION ALL
...
WHERE ltrim(OH.cus_no) IN ('1575', '20938', '122523', '25000', '35000') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND NOT OL.item_no IN ('ADD ON', 'BACKORDER', 
               'CAP EX', 'FIXTURE REQUEST', 'INITIAL DIV 01', 'INITIAL RM', 'INITIAL SC', 'INITIAL WNM', 'PROTOTYPE METAL', 'PROTOTYPE PLASTIC', 'PROTOTYPE WOOD', 
               'REVIEW ITEM', 'SAMPLE') AND OH.oe_po_no IS NOT NULL AND NOT OH.cus_alt_adr_cd IS NULL AND NOT OH.ship_to_addr_2 LIKE 'PO BOX%' AND 
               NOT shipping_dt IS NULL AND OH.ship_to_addr_4 LIKE '%,%' AND isnumeric(cus_alt_adr_cd) = 1 AND isnumeric(oe_po_no) = 1 AND 
               qty_to_ship > 0 /*Exclude split shipment orders*/ AND (OH.ord_no + IM.item_no) NOT IN ('  680261BAK-695 OBV-097', '  680102BAK-ARTBRDE 97', 
               '  680261OBP-1822BSOBV97', '  680524MDWM-0015 SB', '  680524MDWM-0002 SB', '  680524MDWM-0003 SB', '  680524MDWM-0001 SB',
               '  681451BAK-707 C OBV97', '  681451BAK-707EC OBV97', '  680451BAK-ARTBRDE 97', '  681591BAK-707EC OBV97')
...
/*Order Acknowledgement*/ SELECT '838115' AS 'Supplier Acct Number', CAST(OH.oe_po_no AS int) AS 'Wal-Mart PO Number', 
               ...
               CASE WHEN OH.ord_no + OL.item_no = '  680451BAK-ARTBRDE 97' THEN 2
                    WHEN OH.ord_no + OL.item_no = '  681451BAK-707EC OBV97' THEN 6
                    WHEN OH.ord_no + OL.item_no = '  681451BAK-707 C OBV97' THEN 3
                    WHEN OH.ord_no + OL.item_no = '  681591BAK-707EC OBV97' THEN 8  
                    ELSE cast(OL.qty_ordered AS int) 
               END AS 'Supplier Quantity Ordered', 0 AS 'Supplier Quantity Shipped', 'A' AS 'Status', 
...
WHERE ltrim(OH.cus_no) IN ('1575', '20938', '122523', '25000', '35000', '22056') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND NOT OL.item_no IN ('ADD ON', 
               ...
               isnumeric(oe_po_no) = 1 AND OL.shipped_dt IS NULL /*Exclude split shipment orders*/ AND (OH.ord_no + IM.item_no) NOT IN ('  680261BAK-695 OBV-097', 
               '  680102BAK-ARTBRDE 97', '  680261OBP-1822BSOBV97', '  680524MDWM-0015 SB', '  680524MDWM-0002 SB', '  680524MDWM-0003 SB', '  680524MDWM-0001 SB')
UNION ALL
/*Fake upload lines for split shipments, etc.  **Be sure to exclude the faked lines from the actual acknowledgements above to avoid duplication***/ SELECT '838115' AS 'Supplier Acct Number',
               ...
               IN ('  670551GRO-011 WM OW') THEN '6' /*WHEN PP.qty > OL.qty_to_ship THEN rtrim(cast(cast(OL.qty_to_ship AS int) AS char)) */ ELSE rtrim(cast(cast(PP.qty AS int) 
               AS char)) END, 'Wal-Mart Item Part Number' = CASE WHEN OL.cus_item_no IS NULL AND rtrim(CI.cus_item_no) IS NULL THEN rtrim(OL.item_no) 
               ...
               rtrim(replace(replace(replace(IM.item_desc_1, ',', ' '), '"', 'in'), '''', 'ft')) AS 'Supplier Item Part Number Description', '' AS 'Wal-Mart Component Part Number', 
               '' AS 'Supplier Component Part Number', '' AS 'Supplier Component Description', CASE WHEN (OH.ord_no + IM.item_no) IN ('  680261OBP-1822BSOBV97') 
               THEN '9' WHEN (OH.ord_no + IM.item_no) IN ('  680524MDWM-0015 SB') THEN '7' WHEN (OH.ord_no + IM.item_no) IN ('  680524MDWM-0002 SB') 
               THEN '3' WHEN (OH.ord_no + IM.item_no) IN ('  680524MDWM-0003 SB') THEN '11' WHEN (OH.ord_no + IM.item_no) IN ('  680524MDWM-0001 SB') 
               THEN '3' ELSE cast(OL.qty_ordered AS int) END AS 'Supplier Quantity Ordered', CASE WHEN (OH.ord_no + IM.item_no) IN ('  680261OBP-1822BSOBV97') 
               THEN '9' WHEN (OH.ord_no + IM.item_no) IN ('  680524MDWM-0015 SB') THEN '7' WHEN (OH.ord_no + IM.item_no) IN ('  680524MDWM-0002 SB') 
               THEN '3' WHEN (OH.ord_no + IM.item_no) IN ('  680524MDWM-0003 SB') THEN '11' WHEN (OH.ord_no + IM.item_no) IN ('  680524MDWM-0001 SB') 
               THEN '3' ELSE cast(OL.qty_ordered AS int) END AS 'Supplier Quantity Shipped', 'C' AS 'Status', '' AS 'Ship Condition', 'I' AS 'Part Type'
FROM  OELINHST_SQL OL INNER JOIN
...
WHERE ltrim(OH.cus_no) IN ('1575', '20938', '122523', '25000', '35000') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND NOT OL.item_no IN ('ADD ON', 'BACKORDER', 
               ...
               'REVIEW ITEM', 'SAMPLE') AND OH.oe_po_no IS NOT NULL AND NOT OH.cus_alt_adr_cd IS NULL AND NOT OH.ship_to_addr_2 LIKE 'PO BOX%' AND 
               NOT shipping_dt IS NULL AND OH.ship_to_addr_4 LIKE '%,%' AND isnumeric(cus_alt_adr_cd) = 1 AND isnumeric(oe_po_no) = 1 AND qty_to_ship > 0 AND 
               (OH.ord_no + IM.item_no) IN ('  680261BAK-695 OBV-097', '  680102BAK-ARTBRDE 97', '  680261OBP-1822BSOBV97', '  680524MDWM-0015 SB', 
               '  680524MDWM-0002 SB', '  680524MDWM-0003 SB', '  680524MDWM-0001 SB', '  670551GRO-011 WM OW') AND (PP.ship_dt > DATEADD(day, - 60, GETDATE()) OR
               PP.ship_dt IS NULL)