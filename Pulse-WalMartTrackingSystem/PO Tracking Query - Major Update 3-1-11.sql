...
SELECT '838115' AS 'Supplier Acct Number', CAST(OH.oe_po_no AS int) AS 'Wal-Mart PO Number', 
                      ...
                      (charindex(',', OH.ship_to_addr_4) + 2), 2) AS 'Ship to State', 
                      'Carrier SCAC (Standard Carrier Alpha Code) Number' = CASE WHEN XX.code IS NULL 
                      THEN
                          (SELECT    rtrim(XX.code)
                            FROM          EDCSHVFL_SQL XX
                            WHERE      OH.ship_via_cd = XX.mac_ship_via) ELSE rtrim(XX.code) END, 
                            rtrim(SH.tracking_no) AS 'Pro Number or Load Number', 
                      rtrim(cast(ltrim(OH.ord_no) AS CHAR)) AS 'Bill of Lading Number',
                      'Pallet ID Number' = 
                      CASE WHEN SH.[Pallet/Carton ID] IS NULL 
						   THEN (CASE WHEN SH.ID < 9999 
									  THEN ('838115F' + '00000' + CONVERT(varchar, SH.ID, 101)) 
									  WHEN SH.ID < 99999 
									  THEN ('838115F' + '0000' + CONVERT(varchar, SH.ID, 101)) 
									  ELSE ('838115F' + '000' + CONVERT(VARCHAR, SH.ID, 101)) 
								 END) 
								  ELSE '838115A' + '0' + rtrim(ltrim(SH.[Pallet/Carton ID]))
						END, 
					'Container Type' = 
					 CASE substring(OL.item_no, 1, 2) 
						  WHEN 'SW' 
						  THEN 'BOX' 
						  ELSE 'PALLET' 
					 END, 'Container Number' = '', 
					 'Quantity in Container' = 
					 CASE WHEN SH.qty IS NULL 
						  THEN rtrim(cast(cast(OL.qty_ordered AS int) AS char)) 
						  WHEN SH.qty > OL.qty_ordered 
						  THEN rtrim(cast(cast(OL.qty_ordered AS int) AS char)) 
                      ELSE rtrim(cast(cast(SH.qty AS int) AS char)) 
                      END, 
                      'Wal-Mart Item Part Number' = 
                      CASE WHEN OL.cus_item_no IS NULL AND rtrim(CI.cus_item_no) IS NULL
						   THEN rtrim(OL.item_no) 
						   WHEN OL.cus_item_no IS NULL 
						   THEN rtrim(CI.cus_item_no) 
						   ELSE rtrim(OL.cus_item_no) 
					  END, rtrim(OL.item_no) AS 'Supplier Item Part Number', rtrim(replace(replace(replace(IM.item_desc_1, ',', ' '), '"', 'in'), '''', 'ft')) AS 'Supplier Item Part Number Description', 
                      ...
                      'Supplier Quantity Shipped' = 
                       cast(OL.qty_to_ship AS int), 'C' AS 'Status', 'Shipped' AS 'Ship Condition', 'I' AS 'Part Type'
FROM      OEORDLIN_SQL OL 
			INNER JOIN OEORDHDR_SQL OH 
				ON OL.ord_no = OH.ord_no  
			INNER JOIN BG_SHIPPED SH
				ON ltrim(OL.ord_no) = ltrim(SH.ord_no) AND OL.item_no = SH.item_no 
			LEFT OUTER JOIN OECUSITM_SQL CI 
				ON OL.item_no = CI.item_no AND OH.cus_no = CI.cus_no 
			LEFT OUTER JOIN EDCSHVFL_SQL XX 
				ON SH.carrier_cd = XX.mac_ship_via 
			LEFT OUTER JOIN imitmidx_sql IM 
				ON OL.item_no = IM.item_no
WHERE     ltrim(OH.cus_no) IN ('1575', '20938', '122523', '25000', '35000') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND NOT OL.item_no IN ('ADD ON', 
                      'BACKORDER', 'CAP EX', 'FIXTURE REQUEST', 'INITIAL DIV 01', 'INITIAL RM', 'INITIAL SC', 'INITIAL WNM', 'PROTOTYPE METAL', 
                      'PROTOTYPE PLASTIC', 'PROTOTYPE WOOD', 'REVIEW ITEM', 'SAMPLE') AND OH.oe_po_no IS NOT NULL AND 
                      NOT OH.cus_alt_adr_cd IS NULL AND NOT OH.ship_to_addr_2 LIKE 'PO BOX%' AND NOT shipping_dt IS NULL AND OH.ship_to_addr_4 LIKE '%,%' AND isnumeric(cus_alt_adr_cd) = 1 AND isnumeric(oe_po_no) = 1
                      /*Conditions just for Open Order Shipped	- Pull orders shipped in last 40 days from Shipped view*/ 
...
SELECT '838115' AS 'Supplier Acct Number', CAST(OH.oe_po_no AS int) AS 'Wal-Mart PO Number', 
                      ...
                      THEN
                          (SELECT     rtrim(XX.code)
                            FROM          EDCSHVFL_SQL XX
                            WHERE      OH.ship_via_cd = XX.mac_ship_via) ELSE rtrim(XX.code) END, rtrim(SH.tracking_no) AS 'Pro Number or Load Number', 
                      rtrim(cast(ltrim(OH.ord_no) AS CHAR)) AS 'Bill of Lading Number', 
                      'Pallet ID Number' = 
                      CASE WHEN SH.[Pallet/Carton ID] IS NULL 
						   THEN (CASE WHEN SH.ID < 9999 
...
WHERE     ltrim(OH.cus_no) IN ('1575', '20938', '122523', '25000', '35000') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND NOT OL.item_no IN ('ADD ON', 
                      ...
                      'PROTOTYPE PLASTIC', 'PROTOTYPE WOOD', 'REVIEW ITEM', 'SAMPLE') AND OH.oe_po_no IS NOT NULL AND 
                      NOT OH.cus_alt_adr_cd IS NULL AND NOT OH.ship_to_addr_2 LIKE 'PO BOX%' AND NOT shipping_dt IS NULL AND OH.ship_to_addr_4 LIKE '%,%' AND isnumeric(cus_alt_adr_cd) = 1 AND isnumeric(oe_po_no) = 1
                      /*Conditions just for History Shipped	- Pull orders shipped in last 40 days from Shipped view*/ 
                      AND (CONVERT(varchar, CAST(rtrim(SH.ship_dt) AS datetime), 101) > DATEADD(day, - 40, GETDATE()))                
UNION ALL
...
WHERE     ltrim(OH.cus_no) IN ('1575', '20938', '122523', '25000', '35000') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND NOT OL.item_no IN ('ADD ON', 
                      'BACKORDER', 'CAP EX', 'FIXTURE REQUEST', 'INITIAL DIV 01', 'INITIAL RM', 'INITIAL SC', 'INITIAL WNM', 'PROTOTYPE METAL', 
                      'PROTOTYPE PLASTIC', 'PROTOTYPE WOOD', 'REVIEW ITEM', 'SAMPLE') AND OH.oe_po_no IS NOT NULL AND 
                      NOT OH.cus_alt_adr_cd IS NULL AND NOT OH.ship_to_addr_2 LIKE 'PO BOX%' AND NOT shipping_dt IS NULL AND OH.ship_to_addr_4 LIKE '%,%' AND isnumeric(cus_alt_adr_cd) = 1 AND isnumeric(oe_po_no) = 1 AND OL.shipped_dt is null
