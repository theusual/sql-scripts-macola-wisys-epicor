SELECT     OH.status AS 'STATUS', CAST(OH.ord_no AS int) AS 'ORDER', CAST(OH.oe_po_no AS int) AS 'PO', CAST(OH.cus_no AS int) AS 'CUSTOMER', 
                      OH.cus_alt_adr_cd AS 'STORE', rtrim(OL.item_no) AS 'ITEM', rtrim(CI.cus_item_no) AS 'CUST_ITEM', OH.shipping_dt AS 'EST_SHIP_DATE', 
                      SH.ship_dt AS 'ACTUAL_SHIP_DATE', OL.promise_dt AS 'PROMISE_DATE', OH.ship_to_name AS 'SHIP_TO_NAME', rtrim(OH.ship_to_addr_2) 
                      ...
                      PL.palletid AS 'PALLET_ID', PL.quantity AS 'SUM_PALLET_QTY', rtrim(OL.item_desc_1) AS 'ITEM_DESC', OL.qty_ordered AS 'QTY_ORDERED', 
                      SH.hand_chg AS 'QTY_SHIPPED', 'MISSING' AS 'ERROR', SH.mode AS 'Dock'
...
WHERE     /* INITIAL WHERE CLAUSE */ ltrim(OH.cus_no) IN ('1575', '20938') AND OH.oe_po_no > '0' AND OH.ord_type = 'O' AND 
                      ...
                      AS datetime), 101) > DATEADD(day, - 1, GETDATE()))) 
ORDER BY SH.mode
