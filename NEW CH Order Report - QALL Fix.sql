SELECT     TOP (100) PERCENT '___' AS LN, Z_IMINVLOC.prod_cat AS Cat, CASE WHEN (IMITMIDX_SQL.extra_1) IS NULL 
                      ...
                      imitmidx_sql.extra_1 IS NULL) AND ((IMITMIDX_SQL.item_note_4 IS NULL) AND (Z_IMINVLOC.qty_on_hand = 0) AND 
                      (ROUND(Z_IMINVLOC.qty_on_ord - z_iminvloc_qall.qty_allocated - (3 * (z_iminvloc.prior_year_usage / 12)), 0) < 0)) 
                      THEN 'NC' ELSE '' END AS [CHECK 90], CASE WHEN (NOT (IMITMIDX_SQL.item_note_4 IS NULL)) AND (Z_IMINVLOC.qty_on_hand > 0) 
                      ...
                      WHEN (IMITMIDX_SQL.item_note_4 IS NULL) AND (Z_IMINVLOC.qty_on_hand = 0) 
                      THEN (ROUND(Z_IMINVLOC.qty_on_ord - z_iminvloc_qall.qty_allocated - (3 * (z_iminvloc.prior_year_usage / 12)), 0)) 
                      ELSE '' END AS [QOH+QOO-QOA-SS-90], Z_IMINVLOC.po_min AS MOQ, '_________' AS [ORDER], IMITMIDX_SQL.item_note_2 AS Supplier, 
...
FROM         dbo.Z_IMINVLOC AS Z_IMINVLOC INNER JOIN
                      ...
                      dbo.Z_IMINVLOC_QALL ON dbo.Z_IMINVLOC_QALL.item_no = Z_IMINVLOC.item_no INNER JOIN
...
WHERE    
 (IMITMIDX_SQL.item_note_1 like '%ch%') AND (NOT (Z_IMINVLOC.prod_cat = '036' OR
                      ...
                      Z_IMINVLOC.prod_cat = '336')) 
                   AND (dbo.Z_IMINVLOC_USAGE.usage_ytd > 0) 
          OR
(IMITMIDX_SQL.item_note_1 like '%ch%')
                    AND (NOT (Z_IMINVLOC.prod_cat = '036' OR
                      Z_IMINVLOC.prod_cat = '037' OR
                      Z_IMINVLOC.prod_cat = '336')) AND (Z_IMINVLOC.qty_on_ord > 0)
          OR
(IMITMIDX_SQL.item_note_1 like '%ch%') AND (NOT (Z_IMINVLOC.prod_cat = '036' OR
                      Z_IMINVLOC.prod_cat = '037' OR
                      Z_IMINVLOC.prod_cat = '336')) AND (dbo.Z_IMINVLOC_QALL.qty_allocated > 0) 
          OR
(IMITMIDX_SQL.item_note_1 like '%ch%') AND (NOT (Z_IMINVLOC.prod_cat = '036' OR
                      Z_IMINVLOC.prod_cat = '037' OR
                      Z_IMINVLOC.prod_cat = '336')) AND (Z_IMINVLOC.prior_year_usage > 0)
          OR
(IMITMIDX_SQL.item_note_1 like '%ch%') AND (NOT (Z_IMINVLOC.prod_cat = '036' OR
                      Z_IMINVLOC.prod_cat = '037' OR
