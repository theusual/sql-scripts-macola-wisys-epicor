...
--Created:	1/12/14			     By:	BG
--Last Updated:	3/17/14			 By:	BG
--Purpose:	View for Inventory Projections Report for Case Refurb Division
--Last changes: 
	...
	WHERE (OH.ord_type = 'O') 
					AND ((CAST(LTRIM(OL.ord_no) AS VARCHAR) + IM2.item_no + CAST(CAST(OL.qty_to_ship AS INT) AS VARCHAR)) NOT IN
					  (SELECT CAST(LTRIM(Ord_no) AS VARCHAR) + item_no + CAST(SUM(Qty) AS VARCHAR) AS Expr1
					   ...
					   GROUP BY Ord_no, Item_no))
					--Test
					--AND IM2.item_no = 'MTY9200'
					--All CR parents
					AND IM2.prod_cat IN ('036', '037', '111', '336', '102')
					OR 
					IM2.item_no IN (SELECT comp_item_no FROM bmprdstr_sql BM JOIN imitmidx_sql IM ON IM.item_no = BM.item_no WHERE 
