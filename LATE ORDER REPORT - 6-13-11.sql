...
WHERE     (NOT (dbo.oeordhdr_sql.oe_po_no = 'BRUCE' OR
                      ...
                      dbo.oeordhdr_sql.oe_po_no = 'WALT')) 
                      AND (dbo.oeordhdr_sql.ord_type = 'O') 
                      AND (CONVERT(varchar, CAST(RTRIM(dbo.oeordhdr_sql.shipping_dt) 
                      AS datetime), 101) < DATEADD(day, - 1, GETDATE())) 
                      AND (NOT (dbo.oeordhdr_sql.ord_no IN ('920450', '920492', '920505'))) 
                      AND (NOT (dbo.oeordlin_sql.user_def_fld_2 LIKE '%SHIPPED%') OR
                      dbo.oeordlin_sql.user_def_fld_2 IS NULL) 
                      AND (NOT (dbo.oeordlin_sql.prod_cat IN ('037', '2', '036', '102', '111', '336'))) 
                      AND (NOT (dbo.oeordlin_sql.loc = 'IT')) 
                      AND (dbo.ARSHTTBL.ord_no IS NULL) 
                      ...
                      AND NOT (dbo.oeordlin_sql.user_def_fld_1 LIKE '%STOCK%') 
						OR dbo.oeordlin_sql.user_def_fld_1 IS NULL) 
					  ...
					  AND (NOT (dbo.oeordlin_sql.loc = 'CAN'))  AND oeordlin_sql.shipped_dt is null                    
