...
SELECT     TOP (100) PERCENT '___' AS LN, Z_IMINVLOC.prod_cat AS Cat, CASE WHEN (IMITMIDX_SQL.extra_1) IS NULL THEN '' ELSE IMITMIDX_SQL.extra_1 END AS Parent, 
                      Z_IMINVLOC.item_no, IMITMIDX_SQL.item_desc_1, IMITMIDX_SQL.item_desc_2, CAST(Z_IMINVLOC.qty_on_ord AS INT) AS QOO, 
                      CAST(Z_IMINVLOC.qty_on_hand AS Int) AS QOH, CAST(QC.[QOH CHECK] AS INT) AS [QOH CHK], '___' AS AQOH, CAST(Z_IMINVLOC.frz_qty AS INT) AS FQTY, 
                      IMITMIDX_SQL.extra_6 AS [CH-US], Z_IMINVLOC_QALL.qty_allocated AS [QALL_ALL], CASE WHEN proj.qty_proj > 0 THEN CAST((dbo.Z_IMINVLOC_QALL.qty_allocated - Proj.qty_proj) AS Int) 
                      ELSE CAST(Z_IMINVLOC_QALL.qty_allocated AS Int) END AS QALL, CASE WHEN proj.qty_proj > 0 THEN CAST(proj.qty_proj AS INT) ELSE '0' END AS QPROJ, 
                      ...
                      (Z_IMINVLOC.qty_on_hand = 0) THEN (ROUND(Z_IMINVLOC.qty_on_ord - z_iminvloc_qall.qty_allocated, 0)) ELSE '' END AS [QOH+QOO-QOA-SS], IMITMIDX_SQL.uom, 
                      ...
                      ((ROUND(Z_IMINVLOC.qty_on_ord - Z_IMINVLOC.qty_on_hand - (3 * CEILING(dbo.Z_IMINVLOC_USAGE.usage_ytd / (DATEDIFF(day, CONVERT(datetime, '01/01/2012', 
                      103), GETDATE()) / 30.5))), 0)) < 0) THEN 'NC' ELSE '' END AS [CHECK 90], CASE WHEN (Z_IMINVLOC.qty_on_hand > 0) 
