/*Last Change: Added BR to location exclusions*/ SELECT [Parent Item] AS [item_no], SUM(qty_to_ship) AS [usage_ytd], 'PARENT' AS [Level], 
...
FROM  (SELECT ol.item_no AS [Parent Item], (OL.qty_to_ship - OL.qty_return_to_stk) AS qty_to_ship
               ...
               WHERE shipped = 'Y' AND ol.loc NOT IN ('CAN', 'IN', 'BR')) AS LVL_USG
