...
--Create temp table
WITH Usage2012 (item_no, usage_qty, loc) AS 
	...
	FROM  (SELECT ol.item_no AS [Parent Item], (OL.qty_to_ship - OL.qty_return_to_stk) AS qty_to_ship, OL.loc
				   ...
				   WHERE YEAR(OH.inv_dt) = '2012' AND ol.loc NOT IN ('CAN', 'IN', 'BR') AND orig_ord_type IN ('O', 'I', 'C') /*Exclude KPB items sold to Inf*/ AND NOT (ltrim(oh.cus_no) 
								  = '22523' AND IM.item_note_3 = 'KPB')
				   UNION ALL
	...
	FROM		  (SELECT ol.item_no AS [Parent Item], BM.comp_item_no[Comp Lvl 1], (BM.qty_per_par * (ol.qty_to_ship - qty_return_to_stk)) 
				   ...
				   WHERE YEAR(OH.inv_dt) = '2012' AND ol.loc NOT IN ('CAN', 'IN', 'BR') AND orig_ord_type IN ('O', 'I', 'C')
				   UNION ALL
				   ...
				   AS [LVL_USG]
	GROUP BY [Comp Lvl 1], LVL_USG.loc
	

SELECT  *--im.item_no, usg.item_no, prior_year_usage, usage_qty, IM.loc, USG.loc
FROM    Usage2011 USG --JOIN IMINVLOC_SQL AS IM ON IM.item_no = USG.item_no AND USG.loc = IM.loc
