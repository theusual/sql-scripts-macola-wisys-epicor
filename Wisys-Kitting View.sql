DECLARE @loc AS char(3) = 'FW'
DECLARE @parentitem AS char(20) = 'test item pur'
DECLARE @kitqty AS int = '100'

SELECT @loc AS Location,
       @parentitem AS [Parent Item],
       CAST(iminvloc_sql.qty_on_hand AS INT) AS [Parent QOH],
       CASE
         WHEN bmprdstr_sql.seq_no = (SELECT MIN(BM.seq_no)
                                     FROM   bmprdstr_sql BM
                                     WHERE  BM.item_no = @parentitem) THEN CAST(@kitqty AS INT)
         WHEN bmprdstr_sql.seq_no IS NULL THEN CAST(@kitqty AS INT)
         ELSE CAST('0' AS INT)
       END AS [Parent Qty To Recv],
       bmprdstr_sql.comp_item_no AS [Component Item],
       CAST(iminvloc_wv.qty_on_hand AS INT) AS [Comp. QOH],
       CAST(( @kitqty * bmprdstr_sql.qty_per_par ) AS INT) AS [Component Qty To Issue]
FROM   imitmidx_sql
