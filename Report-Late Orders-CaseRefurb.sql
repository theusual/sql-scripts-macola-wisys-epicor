--ALTER VIEW BG_LATE_ORDERS_CR AS 

--Created:	4/27/12	 By:	BG
--Last Updated:	4/2/13	 By:	BG
--Purpose:	View for Late Order Report refreshed each morning by production control -- CR only
--Last Change:  Added PO #
...
SELECT TOP (100) PERCENT CONVERT(varchar, CAST(RTRIM(dbo.oeordhdr_sql.shipping_dt) AS datetime), 101) AS [Shipping Date], CONVERT(varchar, 
               ...
               dbo.oeordlin_sql.prod_cat AS [Shop Code], dbo.oeordlin_sql.item_no, dbo.oeordlin_sql.qty_ordered, 
               dbo.oeordlin_sql.user_def_fld_1 AS [Late Order Comments 1], 
               dbo.oeordlin_sql.user_def_fld_2 AS [Late Order Comments 2], 
               dbo.oeordhdr_sql.slspsn_no AS [SALES #], dbo.oeordhdr_sql.cus_alt_adr_cd AS [Store #], 
...
WHERE (NOT (dbo.oeordhdr_sql.oe_po_no = 'BRUCE' OR
               ...
               dbo.oeordhdr_sql.oe_po_no = 'WALT')) AND (dbo.oeordhdr_sql.ord_type = 'O') 
               ...
               AND dbo.oeordlin_sql.prod_cat IN ('037', '2', '036', '102', '111', '336')
