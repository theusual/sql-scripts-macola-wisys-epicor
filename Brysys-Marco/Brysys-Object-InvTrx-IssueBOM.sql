...
IF NOT EXISTS(SELECT item_no FROM dbo.bmprdstr_sql WITH (NOLOCK) WHERE LTRIM(item_no) = LTRIM(@ParentItem))
	BEGIN
	SET @error = 'ERROR:  Item "' + @ParentItem + '" has no components to issue.  No BOMs found for that item.  No action taken.'
...
SELECT 
	...
	@from_lev_no=0,	@from_seq_no=0,@loc=@InvLoc,@doc_type='I',
	@doc_ord_no=NULL,@doc_source=NULL,@prod_type=NULL,
	@unit_cost=0,@old_unit_cost=0,@new_unit_cost=0,@price=0,@build_qty=0,
	@build_qty_per=0,	@amt=0,@landed_cost=0,@ctl_no='        ',@from_ctl_no='        ',	@par_item_no=@ParentItem,
	@trx_dt=CONVERT(DATE,GETDATE()),@trx_tm=CONVERT(TIME,GETDATE()),
	...
	@comment_2=@Comment2,@transfer_code=NULL,@issue_type=NULL,@status=NULL,
	@batch_id='          ',
	@comment=@Comment1,	@comment2=@Comment2, @filler_0003=NULL,@trx_qty_bkord=0,@rev_no=NULL,
...
SET  next_doc_no = (@ord_no + 1 + (SELECT COUNT(*) FROM dbo.bmprdstr_sql WITH (NOLOCK) WHERE item_no = @ParentItem))
--OUTPUT DELETED.next_doc_no AS OldDockNo, INSERTED.next_doc_no AS NewDocNo
where im_ctl_key_1 = 1

------------------------------------------------------------------------------------------------------------------------
--CTE used for calculated variables and to generate sequential number based on RowNum.  Sequ number is added to ord_no 
--to avoid PK violations when inserting (ord_no is PK of iminvtrx)
------------------------------------------------------------------------------------------------------------------------

;WITH CTE AS
(
	SELECT  
		  BM.comp_item_no AS CompItemNo,
		  CAST(BM.qty_per_par AS DECIMAL) * CAST(@ParentQty AS DECIMAL) AS Quantity, 
		  ILCOMP.qty_on_hand AS CompQOH,
          RowNum = ROW_NUMBER() OVER (ORDER BY BM.comp_item_no),
          ILCOMP.avg_cost,
          BM.qty_per_par          
	FROM dbo.bmprdstr_sql BM WITH (NOLOCK)
		JOIN dbo.iminvloc_sql ILCOMP WITH (NOLOCK) ON ILCOMP.item_no = BM.comp_item_no 
	WHERE LTRIM(BM.item_no) = LTRIM(@ParentItem) AND ILCOMP.loc = @InvLoc
)
...
SELECT @source, ' '+CAST(@ord_no + RowNum AS VARCHAR), @ctl_no, @line_no, @lev_no, @seq_no, @from_source, @from_ord_no, 
	  @from_ctl_no, @from_line_no, @from_lev_no, @from_seq_no, CTE.CompItemNo, @par_item_no, @loc, 
	  @trx_dt, @trx_tm, @doc_dt, @doc_type, @doc_ord_no, @doc_source, @cus_no, @vend_no, 
	  @prod_type, CTE.Quantity, CTE.CompQOH, CTE.avg_cost, CTE.avg_cost, CTE.avg_cost, 
	  @price, Quantity, CTE.qty_per_par, (CTE.Quantity * CTE.avg_cost), @landed_cost, @receipt_ord_no, @status, 
	  @jnl, @batch_id, @user_name, 'I '+ CAST(@ord_no+RowNum AS VARCHAR) +'        00000000000                      ', 
	  @comment, @filler_0003, @trx_qty_bkord, @promise_dt, 
	  @rev_no, @deall_amt, 'BRYSYS', 'IM-ISSUEBOM', 0,0,0,0,0,0, @Comment2, @Transfer_code, @issue_type, @filler_0004
FROM  CTE  
...
DECLARE @qty_rtn_ptd decimal(1,0),@Qty_on_hand decimal(1,0),@frz_qty decimal(1,0),
		@tag_qty decimal(1,0),@Qty_bkord decimal(1,0),@Qty_allocated decimal(1,0),
		@Qty_on_ord decimal(1,0),@qty_scrp_ytd decimal(1,0),@qty_scrp_ptd decimal(1,0),
		@usage_ptd decimal(2,0),@usage_ytd decimal(2,0),@qty_returned_ytd decimal(1,0),
		@qty_sld_ptd decimal(1,0),@qty_sold_ytd decimal(1,0)

--Assign variables for IMINVTRX		
SELECT @Qty_on_hand=-1,@loc=@InvLoc

--Update the iminvloc table with the updated accumulators
Update IMINVLOC_SQL 
SET Qty_on_hand = qty_on_hand - (BM.qty_per_par * @ParentQty),
	usage_ptd = usage_ptd + (BM.qty_per_par * @ParentQty), 
	usage_ytd = usage_ytd + (BM.qty_per_par * @ParentQty)
FROM dbo.bmprdstr_sql BM JOIN dbo.iminvloc_sql ILCOMP ON ILCOMP.item_no = BM.comp_item_no 
WHERE LTRIM(BM.item_no) = LTRIM(@ParentItem) and ILCOMP.loc = @loc
...
--------------------------------------
write_log:
--NO log used for this object because all issue transactions are logged in the iminvtrx table.  Any reports requiring a log of issues performed using this object can simply join on the iminvtrx table and use extra_7 = 'BRYSYS' and extra_9 = 'IM-ISSUEBOM'
