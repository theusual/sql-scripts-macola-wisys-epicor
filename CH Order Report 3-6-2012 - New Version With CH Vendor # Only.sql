SELECT TOP (100) PERCENT '___' AS LN, Z_IMINVLOC.prod_cat AS Cat, CASE WHEN (IMITMIDX_SQL.extra_1) IS NULL 
               ...
               CAST(Z_IMINVLOC.frz_qty AS INT) AS FQTY, IMITMIDX_SQL.extra_6 AS [CH-US], 
               ...
               CAST(ROUND(Z_IMINVLOC.prior_year_usage / 12, 0) AS INT) AS PMNTH, CEILING(dbo.Z_IMINVLOC_USAGE.usage_ytd / (DATEDIFF(day, CONVERT(datetime, 
               '01/01/2012', 103), GETDATE()) / 30.5)) AS CMNTH, CAST(Z_IMINVLOC.qty_on_hand - dbo.Z_IMINVLOC_QALL.qty_allocated AS Int) AS [QOH-QOA], 
               CASE WHEN z_iminvloc_usage.usage_ytd > 0 THEN CAST(((qty_on_hand + qty_on_ord) / CEILING((z_iminvloc_usage.usage_ytd / (DATEDIFF(day, 
               CONVERT(datetime, '01/01/2012', 103), GETDATE()) / 30.5)))) AS money) ELSE 0 END AS MOI, CASE WHEN (NOT (imitmidx_sql.extra_1 = 'P') OR
               ...
               IMITMIDX_SQL.uom, CASE WHEN (NOT (imitmidx_sql.extra_1 = 'P') OR
               imitmidx_sql.extra_1 IS NULL OR
               imitmidx_sql.extra_6 = 'CH-US') AND (Z_IMINVLOC.qty_on_hand < 1) AND 
               ((ROUND(Z_IMINVLOC.qty_on_ord - (3 * CEILING(dbo.Z_IMINVLOC_USAGE.usage_ytd / (DATEDIFF(day, CONVERT(datetime, '01/01/2012', 103), GETDATE()) / 30.5))), 
               0)) < 0) THEN 'NC' WHEN (NOT (imitmidx_sql.extra_1 = 'P') OR
               imitmidx_sql.extra_1 IS NULL OR
               imitmidx_sql.extra_6 = 'CH-US') AND (Z_IMINVLOC.qty_on_hand > 0) AND 
               ...
               '01/01/2012', 103), GETDATE()) / 30.5))), 0)) < 0) THEN 'NC' ELSE '' END AS [CHECK 90], CASE WHEN (Z_IMINVLOC.qty_on_hand > 0) 
               ...
               CAST(ROUND(Z_IMINVLOC.std_cost, 2, 0) AS DECIMAL(8, 2)) AS SC, IMITMIDX_SQL.drawing_release_no AS [Dwg #], 
...
WHERE (IMITMIDX_SQL.item_no IN (SELECT item_no FROM POORDLIN_SQL WHERE LTRIM(vend_no) IN (SELECT vend_no FROM   dbo.BG_CH_Vendors) OR LTRIM(vend_no) = '7224')) AND (NOT (Z_IMINVLOC.prod_cat = '036' OR
               Z_IMINVLOC.prod_cat = '037' OR
               Z_IMINVLOC.prod_cat = '336')) AND (dbo.Z_IMINVLOC_USAGE.usage_ytd > 0) OR
               (IMITMIDX_SQL.item_no IN (SELECT item_no FROM POORDLIN_SQL WHERE LTRIM(vend_no) IN (SELECT vend_no FROM   dbo.BG_CH_Vendors) OR LTRIM(vend_no) = '7224')) AND (NOT (Z_IMINVLOC.prod_cat = '036' OR
               Z_IMINVLOC.prod_cat = '037' OR
