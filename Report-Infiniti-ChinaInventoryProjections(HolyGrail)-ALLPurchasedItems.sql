--ALTER VIEW BG_Inventory_Projections_AllPurchased_Infiniti  AS

-- CREATED:  7/13/12
-- UPDATED:  02/05/13      BY: BG
-- PURPOSE: Inventory projections report for Inf
...
--Item QOH's
	SELECT DISTINCT 
				  ...
				  'QOH' AS STORE, 'QOH' AS [PARENT ITEM (ONLY ON SALES)], IM2.item_note_4 AS ESS, USG.usage_ytd, '' AS [OrdType2]
...
--PO's
	SELECT PL.item_no AS ITEM, IM2.item_desc_1 AS ItemDesc1, IM2.item_desc_2 AS ItemDesc2, 'PO' AS [PO/SLS], 
		/*CASE WHEN NOT (PL.user_def_fld_2 IS NULL) 
				  THEN PL.user_def_fld_2 WHEN PL.user_def_fld_2 IS NULL AND NOT (PL.extra_8 IS NULL) AND LEN(PL.extra_8) = 10 THEN CONVERT(varchar(10), 
				  DATEADD(day, 28, PL.extra_8), 101) WHEN PL.user_Def_fld_2 IS NULL AND (PL.extra_8 IS NULL OR
				  ...
				  101) END AS [SHP/RECV DT]*/ 
				  CASE WHEN NOT (PL.user_def_fld_2 IS NULL) THEN PL.user_def_fld_2 
					   ELSE CONVERT(varchar(10), DATEADD(DAY, 10, PH.ord_dt), 101) 
				  END AS [SHP/RECV DT], 
				  CAST(PL.qty_ordered - PL.qty_received - PL.qty_released AS int) AS QTY, '' AS [PROJ QOH], CAST(LTRIM(SUBSTRING(PL.ord_no, 1, 6)) AS VARCHAR) 
				  AS [ORDER], AP.vend_name AS [VEND/CUS], CONVERT(varchar(10), PH.ord_dt, 101) AS [ORDER DATE], PL.user_def_fld_1 AS [CONTAINER INFO], PL.user_def_fld_3 AS [Container Ship To], PS.ship_to_desc AS [TRANSFER TO], IM2.prod_cat AS [PROD CAT], LTRIM(PH.vend_no) AS [CUS NO], 
	...
	FROM  dbo.apvenfil_sql AS AP INNER JOIN
				  ...
				  dbo.imitmidx_sql AS IM2 ON IM2.item_no = PL.item_no INNER JOIN
					  (SELECT DISTINCT PL.item_no
					   ...
					   WHERE (PH.ord_dt > DATEADD(DAY, - 365, GETDATE())) OR QALL.qty_allocated > 0) AS PURCH_LAST_YR ON PURCH_LAST_YR.item_no = IM2.item_no INNER JOIN
...
--Sales Orders Components
	...
	WHERE (OH.ord_type = 'O') --AND (IM2.prod_cat NOT IN ('')) AND (OL.loc NOT IN ('BR', 'IN', 'CAN')) 
...
--Sales Orders Parents
	SELECT OL.item_no AS ITEM, IM2.item_desc_1 AS ItemDesc1, IM2.item_desc_2 AS ItemDesc2, 'SALE' AS [PO/SLS], CASE WHEN CONVERT(varchar(10), 
				  ...
				  OH.cus_alt_adr_cd AS STORE, OL.item_no AS [PARENT ITEM ON SALES ORD], IM2.item_note_4 AS ESS, USG.usage_ytd, OH.user_def_fld_3
	FROM  dbo.oeordhdr_sql AS OH INNER JOIN
				  dbo.oeordlin_sql AS OL ON OL.ord_no = OH.ord_no INNER JOIN
				  dbo.imitmidx_sql AS IM2 ON IM2.item_no = OL.item_no INNER JOIN
					  (SELECT DISTINCT PL.item_no
